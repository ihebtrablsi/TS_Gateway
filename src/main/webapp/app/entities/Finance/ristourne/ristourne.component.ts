...
export class RistourneComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.ristourneService
      ...
      .subscribe(
        (res: HttpResponse<IRistourne[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  ngOnInit(): void {
    ...
    this.registerChangeInRistournes();
  }
  ...
  trackId(index: number, item: IRistourne): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInRistournes(): void {
    this.eventSubscriber = this.eventManager.subscribe('ristourneListModification', () => this.loadPage());
  }

  delete(ristourne: IRistourne): void {
  ...
  protected onSuccess(data: IRistourne[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/ristourne'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
