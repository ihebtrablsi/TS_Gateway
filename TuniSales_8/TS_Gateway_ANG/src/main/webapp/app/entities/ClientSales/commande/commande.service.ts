...
export class CommandeService {
  public resourceUrl = SERVER_API_URL + 'services/clientsales/api/commandes';
  ...
  protected convertDateFromClient(commande: ICommande): ICommande {
    const copy: ICommande = Object.assign({}, commande, {
      dateCommande: commande.dateCommande && commande.dateCommande.isValid() ? commande.dateCommande.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((commande: ICommande) => {
        commande.dateCommande = commande.dateCommande ? moment(commande.dateCommande) : undefined;
      });
    }
    return res;
  }
