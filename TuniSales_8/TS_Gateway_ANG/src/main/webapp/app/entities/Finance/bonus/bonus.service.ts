...
export class BonusService {
  public resourceUrl = SERVER_API_URL + 'services/finance/api/bonuses';
  ...
  protected convertDateFromClient(bonus: IBonus): IBonus {
    const copy: IBonus = Object.assign({}, bonus, {
      dateAttribution: bonus.dateAttribution && bonus.dateAttribution.isValid() ? bonus.dateAttribution.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((bonus: IBonus) => {
        bonus.dateAttribution = bonus.dateAttribution ? moment(bonus.dateAttribution) : undefined;
      });
    }
    return res;
  }
