...
export class IncentiveComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.incentiveService
      ...
      .subscribe(
        (res: HttpResponse<IIncentive[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IIncentive): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInIncentives(): void {
    this.eventSubscriber = this.eventManager.subscribe('incentiveListModification', () => this.loadPage());
  }

  delete(incentive: IIncentive): void {
  ...
  protected onSuccess(data: IIncentive[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
